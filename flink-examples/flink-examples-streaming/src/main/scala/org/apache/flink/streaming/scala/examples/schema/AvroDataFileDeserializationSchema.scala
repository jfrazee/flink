...
@SerialVersionUID(1L)
class AvroDataFileDeserializationSchema[T](avroType: Class[T]) extends DeserializationSchema[T] {
    ...
    override def deserialize(message: Array[Byte]): T = {
      if (message.length == 0) return null.asInstanceOf[T]
      try {
        val bytes = new ByteArrayInputStream(message)
        val stream = new DataFileStream[T](bytes, reader)
        stream.next
      } catch {
        case e: Exception =>
